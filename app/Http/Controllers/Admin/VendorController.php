...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $restaurant->description = $request->address[array_search('default', $request->lang)];
$restaurant->save();
    ...
    public function update(Request $request, Restaurant $restaurant)
    {
        ...
        if($request->name[array_search('default', $request->lang)] == '' ){
                    $validator->getMessageBag()->add('description', translate('messages.default_restaurant_name_is_required'));
                return back()->withErrors($validator)->withInput();
            }
        if($request->description[array_search('default', $request->lang)] == '' ){
                    $validator->getMessageBag()->add('description', translate('messages.default_restaurant_address_is_required'));
                return back()->withErrors($validator)->withInput();
                }
        ...
        $restaurant->name = $request->name[array_search('default', $request->lang)];
        $restaurant->description = $request->description[array_search('default', $request->lang)];
        $restaurant->latitude = $request->latitude;
        $restaurant->longitude = $request->longitude;
          $restaurant->save();
    ...
    public function view($restaurant,Request $request, $tab = null, $sub_tab = 'cash')
    {
        ...
        return view('admin-views.vendor.view.index', compact('restaurant'));
    }
    ...
    {
        $restaurants = Restaurant::latest()->paginate(config('default_pagination'));
    ...
    {
        $data = Restaurant::
      where('wilaya.name', 'like', '%'.$request->q.'%')
        ->limit(8)->get()
    ...
    public function status(Restaurant $restaurant, Request $request)
    {
        ...
        try {   $restaurant->status = $request->status;
        $restaurant?->save();
            //  $vendor = $restaurant?->vendor;if ($request->status == 0) {
    ...
    public function get_restaurantoptions(Request $request)
    {
        $cat = AddOn::withoutGlobalScope(RestaurantScope::class)->where(['restaurant_id' => $request->restaurant_id])->active()->get();
        $res = '';
        foreach ($cat as $row) {
