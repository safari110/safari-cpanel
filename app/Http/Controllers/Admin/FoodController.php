...
class FoodController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'description.*.required' => translate('messages.description_length_warning'),
        ...
        if ($request->hasFile('images')) {
            foreach ($request->file('images') as $image) {
                $uploadedImages[] = Helpers::upload('product/', 'png', $image);
            }
        }

        // Save the image links instead of images
        $food->images = json_encode($uploadedImages);
        ...
        // $food->images = Helpers::upload(dir: 'product/', format: 'png', image: $request->file('image'));
    ...
    public function edit($id)
    {
      ...
      $categories = Category::where(['parent_id' => 0])->get();
        return view('admin-views.product.edit', compact('product', 'product_images','categories'));
    }
    ...
    {      
        ...
        $validator = Validator::make($request->all(), [
            ...
            'longitude' => 'required|numeric|min:-180|max:180',
            'images' => 'required|array|min:1',
            'imagesurl' => 'required|array|min:0',
            'images.*' => 'image|mimes:jpeg,png,jpg,gif', 
            'email' => 'required|email',
            'phone' => 'required',
            'restaurant_id' => 'required',
        ...
        $uploadedImages = json_decode($request->imagesurl,true);
        ...
        $p->images = json_encode($uploadedImages);
    ...
    // {
    //     $foods = Food::withoutGlobalScope(RestaurantScope::class)->with('restaurant')->get();
    //     $res = '';
    ...
    public function get_foods(Request $request)
    {
        $data = Food::withoutGlobalScope(RestaurantScope::class)->with('restaurant')->where('target.name', 'like', '%'.$request->q.'%')
        ->limit(8)->get()
        ->map(function ($res) {
            return [
                'id' => $res->id,
                'text' => $res->name . ' (' . $res->restaurant->name . ')',
            ];
        });
