...
class CategoryController extends Controller
{
    ...
    public function get_products($id, Request $request)
    {
        ...
        $data = CategoryLogic::products(category_id: $id, limit: $request['limit'], offset: $request['offset'], type: $type);
    ...
    public function get_restaurants($id, Request $request)
    {
        ...
        $data = CategoryLogic::restaurants(category_id: $id, limit: $request['limit'], offset: $request['offset'], type: $type);
    ...
    public function get_all_productsbycategoryresto($id, $resto)
    {
        try {
            $data = Helpers::product_data_formatting(CategoryLogic::all_productsbyrestorandcategory($id, $resto), true, false, app()->getLocale());
            return response()->json($data, 200);
        } catch (\Exception $e) {
            return response()->json([], 200);
        }
    }
    public function get_all_productsbycategoryrest($id, $resto, Request $request)
    {
        try {
            $data = Helpers::product_data_formatting(CategoryLogic::all_productsbyrestorandcategory($id, $resto), true, false, app()->getLocale());
            return response()->json($data, 200);
