...
class RestaurantController extends Controller
{
    public function get_restaurants(Request $request, $filter_data="all")
    {
        ...
        $restaurants = RestaurantLogic::get_restaurants( filter:$filter_data, limit: $request['limit'],offset: $request['offset'],type:$type, name:$name  );
        $restaurants['wilaya'] = Helpers::restaurant_data_formatting(data:$restaurants['wilaya'],multi_data: true);
    ...
    public function get_details($id)
    {
        ...
        if($restaurant)
        {
            $category_ids = DB::table('target')
            ->join('categories', 'target.category_id', '=', 'categories.id')
            ->selectRaw('IF((categories.position = "0"), categories.id, categories.parent_id) as categories')
            ->where('target.restaurant_id', $restaurant->id)
