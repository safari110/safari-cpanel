...
use Illuminate\Support\Facades\DB; // Make sure to include this at the top of your file

class ProductLogic
{

    public static function getNearTargets($userLatitude, $userLongitude, $maxDistance)
    {
        // Find nearby targets using Haversine formula
        $query = Food::select(
            DB::raw("target.*,
        ...
        $nearbyTargets = $query->active()->get();
     //  dd($query);
    ...
    public static function get_restaurant__products($restaurant_id, $type = 'all', $name = null)
    {
        $products = Food::active()->type($type)->where('restaurant_id', $restaurant_id)
            ->when(isset($name), function ($query) use ($name) {
                $query->where(function ($q) use ($name) {
                    foreach ($name as $value) {
                        $q->orWhere('name', 'like', "%{$value}%");
