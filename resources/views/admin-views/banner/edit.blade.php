                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="banner_type" name="banner_type" class="form-control" onchange="banner_type_change(this.value)">
                                            <option value="wilaya_wise" {{$banner->type == 'wilaya_wise'? 'selected':'' }}>{{translate('messages.restaurant')}} {{translate('messages.wise')}}</option>
                                            <option value="item_wise" {{$banner->type == 'item_wise'? 'selected':'' }}>{{translate('messages.target')}} {{translate('messages.wise')}}</option>
                                        </select>
                                    </div>
                                    <div class="form-group" id="wilaya_wise">
                                        <label class="input-label" for="exampleFormControlSelect1">{{translate('messages.restaurant')}}<span
                                                class="input-label-secondary"></span></label>
                                        <select name="wilaya_id" class="js-data-example-ajax" id="resturant_ids"  title="Select wilaya">
                                    ...
                                    <div class="form-group" id="item_wise">
                                        <label class="input-label" for="exampleFormControlInput1">{{translate('messages.select')}} {{translate('messages.target')}}</label>
                                        <select name="item_id" id="choice_item" class="form-control js-select2-custom" placeholder="{{translate('messages.select_target')}}">
                                            @if($banner->type=='item_wise')
                                            @php($restaurant = \App\Models\Food::where('id', $banner->data)->first())
                                               @if($restaurant)
                                                   <option value="{{$restaurant->id}}" selected>{{$restaurant->name}}</option>
                                               @endif
                                           @endif
   ...
   // INITIALIZATION OF SELECT2
        ...
        function banner_type_change(order_type) {
           if(order_type=='item_wise')
            ...
            else if(order_type=='wilaya_wise')
            {
                $('#wilaya_wise').show();
                $('#item_wise').hide();
            }
